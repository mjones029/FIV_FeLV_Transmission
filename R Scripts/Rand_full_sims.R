...
#========================================================	
# ---
### title: Random networks, full simulations
...
# 
# What this code does:
# 1. Simulate FeLV transmission on random networks (adapted from "FIV_full_sims.R")
...
# function for simulating a random network
source("R Scripts/simulate_randnet.R")
...
for(q in 1:length(params.to.run)){
  ...
  #### assign random network simulation parameters ####
  ...
  # proportions of those infected that become progressive, regressive, and abortive infections
  ...
  for(z in 1:nsims){
    ...
    #### SIMULATE NETWORK ####
    net3 <- simulate_randnet(net.den = net.den,
                             pop.size = pop.size
    )
    ...
    # save results
    g.name <- paste("Rand_Simulation_Results/g network_rand paramset ", param.set.num, "_sim ",sim.num, ".Rdata", sep = "")
    save(g, file = g.name)
    
    m.name <- paste("Rand_Simulation_Results/m data_rand paramset ", param.set.num, "_sim ", sim.num, ".Rdata", sep = "")
    save(m, file = m.name)
    
    mnew.name <- paste("Rand_Simulation_Results/mnew data_rand paramset ", param.set.num, "_sim ", sim.num, ".Rdata", sep = "")
    save(m.new, file = mnew.name)
    
    presults.name <- paste("Rand_Simulation_Results/presults data_rand paramset ", param.set.num, "_sim ", sim.num, ".Rdata", sep = "")
    ...
    gplot.name <- paste("Rand_Simulation_Results/Simulation_Figures/g plot_rand paramset ", param.set.num, "_sim ", sim.num, ".jpg", sep = "")
    jpeg(gplot.name)
    ...
    eplot.name <- paste("Rand_Simulation_Results/Simulation_Figures/epi plot_rand paramset ", param.set.num, "_sim ", sim.num, ".jpg", sep = "")
    jpeg(eplot.name)
    
    plot(p.results$prop.i, col="red", type='l', main="Simulation of FeLV-like spread on a random network",sub="",xlab="Time (weeks)",ylab="Population numbers",ylim=c(0,1),xlim=c(0, nrow(p.results)))   
    ...
    temp.results <- extract_results(p.results.a = p.results,
                                    ...
                                    c.beta.a = c.beta,
                                    m.list.a = m.list,
                                    end.point = 2 # end point at 2 year mark
    )
  ...
  full.sims.results$param.set <- paste("rand_set_", param.set.num, sep = "")
  
  # Save final results
  full.name <- paste("Rand_Simulation_Results/full set results_rand paramset ", param.set.num, ".Rdata", sep = "")
  save(full.sims.results, file = full.name)s
