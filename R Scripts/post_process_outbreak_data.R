# post_process_outbreak_data
#
#========================================================	
# ---
### title: Process outbreak data involving births/respawning
...
# 
# What this code does:
# 1. Function for processing outbreak data when the outbreak involved respawning ("births")
# The goal is to produce a new "m" matrix where the individuals that occupied a dead ("empty") territory now have their own column


post_process_outbreak_data <- function(m=m){

  m.orig <- m # matrix output from trarns_sim.R function
  ...
  for(j in 1:ncol(m.orig)){
    ...
    # pull out a column
    temp.m <- m.orig[,j]
    # temporarily make vaccinated individuals a different status number so 
    # only 4-->0 transition has a difference of 4
    temp.m[temp.m==6] <- 12
