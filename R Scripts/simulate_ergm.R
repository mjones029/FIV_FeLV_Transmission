...
# 
# What this code does:
# 1. Preliminary function for simulating larger networks from an ERGM model

simulate_ergm <- function(telemetry = tele.ordered,       # telemetry data to use for simulating HR centroids
                          ...
                          net.style = "bestERGM"              # "bestERGM" - determines which ergm to use to generate network; just "bestERGM" right now, but could incorporate other ERGM results
                          ){
  ...
  #### SIMULATION POPULATION REPRESENTATIVE OF FELV-PERIOD FL PANTHERS ####
  ...
  for(i in 1:length(cats)){
    ...
    if(nrow(temp.tele)>=50){
      # if more than 50 points for an individual, down sample to 50
      ...
      # if between 30 and 50 points, keep all
      
      sample.locs <- temp.tele
      tele.mcp_30.50 <- rbind(tele.mcp_30.50, sample.locs)
      
    }else if(nrow(temp.tele)<30){
      # if less than 30, randomly select a single point
  ...
  # generate minimum convex polygon home ranges
  ...
  # calculate quadrats, counts, and proportions per quadrat
  ...
  # assign consistent naming to simulated distance and kin matrices
  ...
  # initialize x to a random undirected network
  ...
  if(net.style=="bestERGM"){
    ...
    print("WARNING: Incorrect ERGM assignment")
  }
